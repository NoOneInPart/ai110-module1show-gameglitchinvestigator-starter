...
#FIX: refactored game logic code to logic_utils.py using Claude
...
#FIX: start a fresh game when the difficulty is switched so the range, secret,
# and attempt limit all stay consistent, authored with Claude
...
# FIX: render the "Attempts left" message after the guess is processed so the
# count isn't one update behind, authored with Claude
attempts_left_box = st.empty()

# FIX: render the debug info after the guess is processed so the values aren't
# one update behind, authored with Claude
debug_box = st.empty()
...
if submit:
    ...
    if not ok:
        ...
        #FIX: do not randomly cast secret to string on even-numbered attempts, authored with Claude
